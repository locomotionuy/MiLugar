	...
	
{
  ...
  try 
	{
    $sql = "UPDATE $tabla SET visitas=$visitas, ultima_visita='$ultima_visita' WHERE id=$id LIMIT 1";
    $conn->exec($sql);
  }
  catch(PDOException $e)
  {
    error_log("error de visitas> " . $e->getMessage());
  }
    ...
    <div class="center">
      <div class="content">
        ...
        <div id="informacion">
          ...
          <?php } ?>
          <?php if($row['mapa']!=="" || $row['latitud']!=0.000000 && $row['longitud']!=0.000000) {  ?>
          <div class="titulos"><?php echo $texto_label_ubicacion ?></div>
          <div class="map">            
            <iframe src="<?php if($row['latitud']!=0.000000 && $row['longitud']!=0.000000) echo "https://maps.google.com/maps?q=".$row['latitud'].",".$row['longitud']."&hl=es;z=14&amp;output=embed"; else echo $row['mapa']; ?>" width="100%" height="400px" frameborder="0" style="border:0;" allowfullscreen></iframe>
          </div>
          <?php } ?>
        </div>
      </div>
